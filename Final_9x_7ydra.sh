...
# ############################################################################
# ## NEW FUNCTION: check_ssl_certs_nmap
# ############################################################################
# This function uses nmap to check the SSL certificate on port 443 for each IP.
# It extracts domains from the certificate's Subject Alternative Name (SAN)
# and saves any that match the main scope.
check_ssl_certs_nmap() {
  local main_scope_domain=$1
  local ip_list_file=$2

  echo -e "\n${PURPLE}============================================================${RESET}"
  echo -e "${BLUE}[+] Checking SSL Certificates with Nmap for In-Scope Domains${RESET}"
  echo -e "${PURPLE}============================================================${RESET}"

  # Check if nmap is installed
  if ! command -v nmap &> /dev/null; then
    echo -e "${RED}[!] Nmap is not installed. Skipping SSL certificate check.${RESET}"
    return 1
  fi

  # Check if there are any IPs to scan
  if [ ! -s "$ip_list_file" ]; then
    echo -e "${YELLOW}[!] No IP addresses found to scan with Nmap.${RESET}"
    return 1
  fi
  
  local ip_count
  ip_count=$(wc -l < "$ip_list_file")
  echo -e "${CYAN}[+] Main Scope defined as: *.$main_scope_domain${RESET}"
  echo -e "${GREEN}[+] Processing $ip_count unique IPs with Nmap...${RESET}"

  local ip_processed=0
  while IFS= read -r ip; do
    [[ -z "$ip" ]] && continue
    ip_processed=$((ip_processed + 1))
    echo -e "${BLUE}[+] Scanning IP ${ip_processed}/${ip_count}: ${GREEN}$ip${RESET}"
    
    # Run nmap with the ssl-cert script on port 443
    # Use -oG - to get grep-able output, which is easier to parse in bash
    nmap_output=$(nmap -p 443 --script ssl-cert -T4 -oG - "$ip" 2>/dev/null)
    
    # Check if we got any output for Subject Alternative Name
    if echo "$nmap_output" | grep -q "Subject Alternative Name"; then
      # Parse the SAN line to extract DNS names
      cert_domains=$(echo "$nmap_output" | grep "Subject Alternative Name" | sed -e 's/.*Subject Alternative Name: //g' -e 's/DNS://g' | tr ',' '\n' | tr -d ' ')

      found_count=0
      while IFS= read -r cert_domain; do
        [[ -z "$cert_domain" ]] && continue

        # Check if the discovered domain is within the main scope
        if [[ "$cert_domain" == "$main_scope_domain" || "$cert_domain" == *".$main_scope_domain" ]]; then
          echo -e "${GREEN}     - Found in-scope domain in cert: ${CYAN}$cert_domain${RESET}"
          # Save the in-scope domain to our new file
          echo "$cert_domain" >> "$OUTPUT_DIR/ssl_cert_discovered_domains.txt"
          found_count=$((found_count + 1))
        else
          echo -e "${YELLOW}     - Found out-of-scope domain: $cert_domain${RESET}"
        fi
      done <<< "$cert_domains"
      
      if [[ $found_count -eq 0 ]]; then
         echo -e "${YELLOW}     - SSL cert found, but no new in-scope domains.${RESET}"
      fi
    else
      echo -e "${YELLOW}     - No SSL certificate information found on port 443.${RESET}"
    fi
  done < "$ip_list_file"
...
# ############################################################################
# ## NEW FUNCTION: get_base_domain
# ############################################################################
# A helper function to extract the registrable domain (e.g., target.com)
# from a subdomain (e.g., sub.dev.target.com).
# This is a simple heuristic and might not cover all edge cases (like .co.uk).
get_base_domain() {
  local domain=$1
  # This uses rev and cut to get the last two parts of the domain.
...
check_prerequisites() {
  ...
  # Check if nmap is installed
  if ! command -v nmap &> /dev/null; then
    echo -e "${YELLOW}[!] Warning: nmap is not installed. SSL certificate checks will be skipped.${RESET}"
  fi
...
print_usage() {
  ...
  # NEW OPTION
  echo -e "   ${GREEN}--skip-nmap-ssl${RESET}            Skip Nmap SSL certificate check"
...
touch "$OUTPUT_DIR/ssl_cert_discovered_domains.txt"
...
# ############################################################################
# ## NEW LOGIC: Determine main scope domain
# ############################################################################
# Read the first valid domain from the input file to determine the main scope
FIRST_DOMAIN=$(grep -v "^#" "$INPUT_FILE" | grep -v "^$" | head -n 1)
MAIN_SCOPE_DOMAIN=$(get_base_domain "$FIRST_DOMAIN")
...
# ############################################################################
# ## NEW LOGIC: Calling the Nmap SSL check function
# ############################################################################
# Process unique IPs with Nmap for SSL certs if not skipped
if [[ "$SKIP_NMAP_SSL" != "true" ]]; then
  UNIQUE_IPS_FILE="$OUTPUT_DIR/all_ips.txt"
  check_ssl_certs_nmap "$MAIN_SCOPE_DOMAIN" "$UNIQUE_IPS_FILE"
else
  echo -e "${YELLOW}[!] Nmap SSL Certificate check disabled${RESET}"
fi
...
# Also clean up the new file
for file in "$OUTPUT_DIR/all_ips.txt" "$OUTPUT_DIR/all_urls.txt" "$OUTPUT_DIR/all_endpoints.txt" "$OUTPUT_DIR/origin_servers.txt" "$OUTPUT_DIR/ssl_cert_discovered_domains.txt"; do
  if [[ -f "$file" ]]; then
...

{
  ...
  echo "Main Scope Domain: $MAIN_SCOPE_DOMAIN"
  echo ""
  echo "## Statistics"
  ...
  echo "- Unique URLs Found: $(wc -l < "$OUTPUT_DIR/all_urls.txt")"
  echo "- Unique Endpoints Found: $(wc -l < "$OUTPUT_DIR/all_endpoints.txt")"
  echo "- Live Endpoints Found: $(wc -l < "$OUTPUT_DIR/live_endpoints.csv" | awk '{print $1-1}')"  # Subtract 1 for header
  echo "- In-Scope Domains from SSL Certs: $(wc -l < "$OUTPUT_DIR/ssl_cert_discovered_domains.txt")"
  echo ""
  echo "## Service Usage"
  echo "- AlienVault OTX API: $([[ "$SKIP_ALIENVAULT" != "true" ]] && echo "Enabled" || echo "Disabled")"
  ...
  echo "- Nmap SSL Check: $([[ "$SKIP_NMAP_SSL" != "true" ]] && echo "Enabled" || echo "Disabled")"
